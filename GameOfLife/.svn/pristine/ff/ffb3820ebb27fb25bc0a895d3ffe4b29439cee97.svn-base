...
public class GameOfLifeMain {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// We know these shouldn't technically be capitalized, but we are doing
		// it
		// anyway
		...
		Timer mrClicker = new Timer(DELAY, updateButton);
		mrClicker.start();
	...
	private static ArrayList<Coordinates> stringToCoords(String input, int x_offset, int y_offset) {

		/**
		 * Takes a String and turns it into an ArrayList of Coordinates. String
		 * has 0's to represent dead cells, 1 for live cells. For muti-line
		 * setups, the string should contain 'x' characters between each line.
		 * 
		 * For example, this plus sign: 010 111 010
		 * 
		 * Would be entered as: "010x111x010"
		 */
		ArrayList<Coordinates> coordList = new ArrayList<Coordinates>();
	...
	private static ArrayList<Coordinates> stringArrayListToCoords(ArrayList<String> stringList, int x_offset,
			int y_offset) {
		ArrayList<Coordinates> coordList = new ArrayList<Coordinates>();

		/**
		 * Takes a block of strings, turns them into coordinates. Useful for
		 * taking preexisting designs and playing with them.
		 * 
